 ...
 * Change Logs:
 * Date            Author          Notes
 * 2017-07-11      aaron           first version
 ************************************************************************/
/*** Including ***/
...
static struct rt_stm32_eth s88e6060_device;
...
void ETH_IRQHandler(void)
{
    ...
    if(status & ETH_DMA_IT_NIS)
    {
        ...
        if((status & ier) & ETH_DMA_IT_R) /* packet reception */
        {
            ...
            eth_device_ready(&(s88e6060_device.parent));
...
static rt_err_t s88e6060_init(rt_device_t dev)
{
    ...
    ETH_MACAddressConfig(ETH_MAC_Address0, (u8*)&s88e6060_device.dev_addr[0]);
...
static rt_err_t s88e6060_open(rt_device_t dev, rt_uint16_t oflag)
{
	return RT_EOK;
}

static rt_err_t s88e6060_close(rt_device_t dev)
{
	return RT_EOK;
}

static rt_size_t s88e6060_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
	rt_set_errno(-RT_ENOSYS);
	return 0;
}

static rt_size_t s88e6060_write (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
	rt_set_errno(-RT_ENOSYS);
	return 0;
}

static rt_err_t s88e6060_control(rt_device_t dev, rt_uint8_t cmd, void *args)
{
	switch(cmd)
	{
	case NIOCTL_GADDR:
		/* get mac address */
		if(args) rt_memcpy(args, s88e6060_device.dev_addr, 6);
...
/* transmit packet. */
rt_err_t s88e6060_tx( rt_device_t dev, struct pbuf* p)
{
    struct pbuf* q;
    rt_uint32_t offset;
...
/* reception packet. */
struct pbuf *s88e6060_rx(rt_device_t dev)
{
    struct pbuf* p;
...
void rt_hw_88e6060_init(u8* mac)
{
	uint8_t smi_dev_addr = 0xFF;
    GPIO_Configuration();
    NVIC_Configuration();

    s88e6060_device.ETH_Speed = ETH_Speed_100M;
    s88e6060_device.ETH_Mode  = ETH_Mode_FullDuplex;

    /* OUI 00-80-E1 STMICROELECTRONICS. */
    s88e6060_device.dev_addr[0] = mac[0];
    s88e6060_device.dev_addr[1] = mac[1];
    s88e6060_device.dev_addr[2] = mac[2];
	
	s88e6060_device.dev_addr[3] = mac[3];
    s88e6060_device.dev_addr[4] = mac[4];
    s88e6060_device.dev_addr[5] = mac[5]; 
    s88e6060_device.parent.parent.init       = s88e6060_init;
    s88e6060_device.parent.parent.open       = s88e6060_open;
    s88e6060_device.parent.parent.close      = s88e6060_close;
    s88e6060_device.parent.parent.read       = s88e6060_read;
    s88e6060_device.parent.parent.write      = s88e6060_write;
    s88e6060_device.parent.parent.control    = s88e6060_control;
    s88e6060_device.parent.parent.user_data  = RT_NULL;

    s88e6060_device.parent.eth_rx     = s88e6060_rx;
    s88e6060_device.parent.eth_tx     = s88e6060_tx;

    /* init tx semaphore */
    rt_sem_init(&tx_wait, "tx_wait", 0, RT_IPC_FLAG_FIFO);

    /* register eth device */
    eth_device_init(&(s88e6060_device.parent), "e0");

    s88e6060_init((rt_device_t)&s88e6060_device);
    ...
    #ifdef ETH_DEBUG
	{
		...
		for(i = 0; i<=0x1F; i++)
		{
			temp = ETH_ReadPHYRegister(i, PHY_BIR);
			
			if(temp != 0xFFFF)
			{
				STM32_ETH_PRINTF("PHY Address: %02x, And Value: %02x\r\n", i, temp );
			}
		}
	}   
	
	{
		STM32_ETH_PRINTF("*****************Set Before*****************\n");
		STM32_ETH_PRINTF("**************PHY Registers**************\n");
		for(smi_dev_addr = SMI_DEV_ADDR_BASIC; smi_dev_addr <SMI_DEV_ADDR_BASIC + PHY_Num_Max; smi_dev_addr++)
		{
			STM32_ETH_PRINTF("phy %d  CR: 0x%04x  SR: 0x%04x \r\n", smi_dev_addr, ETH_ReadPHYRegister(smi_dev_addr, PHY_BCR), ETH_ReadPHYRegister(smi_dev_addr, PHY_BSR));
		}
		STM32_ETH_PRINTF("**************PORT Registers**************\n");
		for(smi_dev_addr = SWITCH_PORT_ADDR; smi_dev_addr <SWITCH_PORT_ADDR + PORT_Num_Max; smi_dev_addr++)
		{
			STM32_ETH_PRINTF("port %d  CR: 0x%04x  SR: 0x%04x \r\n", smi_dev_addr, ETH_ReadPHYRegister(smi_dev_addr, PORT_CONTROL_REG_ADDR), ETH_ReadPHYRegister(smi_dev_addr, PORT_STATUS_REG_ADDR));
		}
	}
	#endif

	//ÉèÖÃ½»»»Ð¾Æ¬88E6060¼Ä´æÆ÷
	{
		for(smi_dev_addr = SMI_DEV_ADDR_BASIC; smi_dev_addr <SMI_DEV_ADDR_BASIC + PHY_Num_Max; smi_dev_addr++)
		{
			ETH_WritePHYRegister(smi_dev_addr, PHY_BCR, PHY_Reset);
			ETH_WritePHYRegister(smi_dev_addr, PHY_BCR, PHY_AutoNegotiation);
//			ETH_WritePHYRegister(smi_dev_addr, PHY_BCR, PHY_FULLDUPLEX_100M);
//			ETH_WritePHYRegister(smi_dev_addr, PHY_BCR, PHY_Restart_AutoNegotiation);
		}
		
		for(smi_dev_addr = SWITCH_PORT_ADDR; smi_dev_addr <SWITCH_PORT_ADDR + PORT_Num_Max; smi_dev_addr++)
		{
			ETH_WritePHYRegister(smi_dev_addr, PORT_CONTROL_REG_ADDR, PORT_Ctl_Forwarding);			
		}
	}
	
    #ifdef ETH_DEBUG
	{
		STM32_ETH_PRINTF("*****************Set After*****************\n");
		STM32_ETH_PRINTF("**************PHY Registers**************\n");
		for(smi_dev_addr = SMI_DEV_ADDR_BASIC; smi_dev_addr <SMI_DEV_ADDR_BASIC + PHY_Num_Max; smi_dev_addr++)
		{
			STM32_ETH_PRINTF("phy %d  CR: 0x%04x  SR: 0x%04x \r\n", smi_dev_addr, ETH_ReadPHYRegister(smi_dev_addr, PHY_BCR), ETH_ReadPHYRegister(smi_dev_addr, PHY_BSR));
